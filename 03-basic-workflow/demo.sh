...
REPO_DIR="exercise-basic-workflow"
rm -rf "$REPO_DIR"
mkdir -p "$REPO_DIR" && cd "$REPO_DIR"
git init > /dev/null

print_step 1 "Create an initial file"
execute_command "echo 'db_host: 10.0.0.1' > config.yml"
execute_command "git status"
wait_user

print_step 2 "Add file to staging area"
execute_command "git add config.yml"
execute_command "git status"
wait_user

print_step 3 "Make your first commit"
execute_command "git commit -m 'feat: add initial database configuration'"
execute_command "git log --oneline"
wait_user

print_step 4 "Modify the file and check diff"
execute_command "echo 'db_port: 5432' >> config.yml"
execute_command "git diff"
wait_user

print_step 5 "Finish the update"
execute_command "git add config.yml"
execute_command "git commit -m 'feat: add database port'"
execute_command "git log --oneline"
